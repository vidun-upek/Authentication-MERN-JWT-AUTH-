...
export const sendVerifyOtp = async (req, res) => {
    try {
        ...
        user.verifyotp = otp;
        user.verifyOtpExpiryAt = Date.now() + 24 * 60 * 60 * 1000; // Corrected schema property
        await user.save();
...
export const verifyEmail = async (req, res) => {
    ...
    try {
        ...
        // SECURITY FIX: Find the user by their ID AND the OTP
        const user = await userModel.findOne({ _id: userId, verifyotp: otp }); 

        // Step 3: if no user found (ID and OTP don't match)
        if (!user) {
